...
test_netbox_unit_tests() {
  ...
  $doco run --rm netbox python manage.py makemigrations netbox_users --check
  $doco run --rm netbox python manage.py test netbox_users
}
...
test_cleanup() {
  ...
  $doco down -v
  $doco rm -fsv
  docker image rm docker.io/library/netbox-users-netbox || echo ''
}
